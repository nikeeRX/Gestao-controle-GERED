...
# NOVO MODELO: ATA DE REUNIÃO
class AtaReuniao(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    assunto = db.Column(db.String(200), nullable=False)
    data_criacao = db.Column(db.Date, default=datetime.utcnow().date)
    topicos = db.Column(db.Text, nullable=False) # Salva os tópicos com quebra de linha

with app.app_context():
    db.create_all()

# ==========================================
# 2. COMPONENTES VISUAIS (HTML EMBUTIDO)
# ==========================================
MENU_TOPO = """
...
<body class="bg-light">
    ...
    <div class="container" style="max-width: 1000px;">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
                <a href="{{ link_whatsapp }}" target="_blank" class="btn btn-success fw-bold me-2">📱 Enviar Resumo Zap</a>
        ...
        <div class="accordion" id="accordionDemandas">
                ...
                <h2 class="accordion-header" id="heading{{ demanda.id }}">
                    <button class="accordion-button collapsed rounded" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ demanda.id }}">
                        <div class="d-flex align-items-center w-100 justify-content-between me-3">
                            <div>
                                <span class="badge bg-dark me-2">{{ demanda.area }}</span>
                                ...
                                {% endif %}
                                <span class="ms-2 text-dark fw-semibold text-truncate d-inline-block" style="max-width: 350px;">{{ demanda.descricao }}</span>
                            </div>
                            <div>
                                <span class="badge bg-light text-dark border me-3">{{ demanda.status }}</span>
                                <small class="text-danger fw-bold">📅 Prev: {{ demanda.data_prevista.strftime('%d/%m/%Y') }}</small>
                            </div>
                        </div>
                    </button>
                </h2>
                        ...
                        <form action="/atualizar/{{ demanda.id }}" method="POST">
                            <div class="row mb-3">
                                <div class="col-md-4 border-end">
                                    <label class="form-label fw-bold text-secondary">Atualizar Status</label>
                                ...
                                <div class="col-md-8">
                                    ...
                                    <p class="bg-light p-2 rounded small text-dark">{{ demanda.descricao }}</p>
                                </div>
                            </div>
                            
                            <h6 class="fw-bold text-secondary mb-2">Checklist</h6>
            ...
            {% else %}
            <div class="text-center py-5 bg-white rounded shadow-sm"><p class="text-muted mb-0 fs-5">Sem demandas pendentes! 🚀</p></div>
            {% endfor %}
        </div>
    </div>
</body>
...
<body class="bg-light">
    ...
    <div class="container" style="max-width: 1000px;">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="mb-0 text-secondary">📁 Atas de Reunião</h3>
            <a href="/nova_ata" class="btn btn-info text-white fw-bold">+ Criar Nova Ata</a>
        </div>

        <form method="GET" action="/atas" class="mb-4">
            <div class="input-group shadow-sm">
                <input type="text" name="busca" class="form-control" placeholder="Pesquisar por assunto ou tópicos discutidos..." value="{{ busca }}">
                ...
                <div class="card shadow-sm border-0 h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <h5 class="card-title text-dark fw-bold text-truncate" title="{{ ata.assunto }}">{{ ata.assunto }}</h5>
                        ...
                        <a href="/gerar_pdf_ata/{{ ata.id }}" class="btn btn-sm btn-outline-danger fw-bold mt-2">
                            📄 Baixar PDF
                        </a>
                    </div>
                </div>
            ...
            <div class="card-body p-4">
                <form method="POST">
                    <div class="mb-3">
                        ...
                        <input type="text" name="assunto" class="form-control" placeholder="Ex: Alinhamento de Metas GERED" required>
                    </div>
                    <div class="mb-4">
                        <label class="form-label fw-bold">Tópicos Discutidos (Um por linha)</label>
                        <div class="form-text text-muted mb-2">Pressione 'Enter' para separar cada tópico. O sistema formatará automaticamente em lista no PDF.</div>
                        <textarea name="topicos" class="form-control" rows="8" placeholder="1. Definimos o prazo da demanda...\n2. O gerente solicitou alteração no relatório...\n3. Alinhado retorno com equipe técnica..." required></textarea>
                    </div>
                    <div class="d-flex justify-content-between">
...
# Restante das telas (Nova Demanda)...
TELA_NOVA_DEMANDA = """<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Nova Demanda</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"></head><body class="bg-light">""" + MENU_TOPO + """<div class="container" style="max-width: 800px;"><div class="card shadow-sm border-0"><div class="card-header bg-dark text-white p-3"><h4 class="mb-0">Cadastrar Nova Demanda</h4></div><div class="card-body p-4"><form method="POST"><div class="row mb-3"><div class="col-md-6"><label class="form-label fw-bold">Área Responsável</label><select name="area" class="form-select" required><option value="CODER">CODER</option><option value="COCAP">COCAP</option><option value="CONEC">CONEC</option><option value="GERED">GERED</option><option value="EXTERNO">EXTERNO</option></select></div><div class="col-md-6"><label class="form-label fw-bold">Prioridade</label><select name="prioridade" class="form-select" required><option value="Mínimo">Mínimo</option><option value="Médio" selected>Médio</option><option value="Alto">Alto</option><option value="Extremo">Extremo</option></select></div></div><div class="mb-3"><label class="form-label fw-bold">Descrição da Demanda</label><textarea name="descricao" class="form-control" rows="3" required></textarea></div><div class="row mb-4"><div class="col-md-6"><label class="form-label fw-bold">Data de Início</label><input type="date" name="data_inicio" class="form-control"></div><div class="col-md-6"><label class="form-label fw-bold text-danger">Data Prevista para Conclusão</label><input type="date" name="data_prevista" class="form-control border-danger" required></div></div><div class="bg-light p-3 rounded border"><div class="d-flex justify-content-between align-items-center mb-3"><h5 class="mb-0">Checklist</h5><button type="button" class="btn btn-sm btn-outline-dark" onclick="adicionarPasso()">+ Passo</button></div><div id="checklist-container"><input type="text" name="passo_checklist[]" class="form-control mb-2" placeholder="Primeira etapa..."></div></div><div class="d-flex justify-content-between mt-4"><a href="/" class="btn btn-secondary">Cancelar</a><button type="submit" class="btn btn-success px-5 fw-bold">Salvar Demanda</button></div></form></div></div></div><script>function adicionarPasso() {const c = document.getElementById('checklist-container');const i = document.createElement('input');i.type='text';i.name='passo_checklist[]';i.className='form-control mb-2';i.placeholder='Próxima etapa...';c.appendChild(i);}</script></body></html>"""

# ==========================================
# 3. ROTAS E LÓGICA DO SISTEMA
# ==========================================

# Rotas de Demandas (Mesmo que antes)
@app.route('/')
def index():
    ...
    for d in demandas_ativas:
        ...
        texto_whats += f"{ico} *[{d.prioridade.upper()}]* {d.descricao[:60]}...\n"
...
# --- NOVAS ROTAS: ATAS DE REUNIÃO ---

@app.route('/atas')
def lista_atas():
    busca = request.args.get('busca', '')
    if busca:
        # Pesquisa tanto no título quanto nos tópicos
...
def nova_ata():
    if request.method == 'POST':
        nova = AtaReuniao(
            ...
            topicos=request.form['topicos']
        )
        db.session.add(nova)
        db.session.commit()
        # Redireciona de volta pra lista de atas
...
def gerar_pdf_ata(id):
    ...
    # Montando o PDF do zero
    pdf = FPDF()
    pdf.add_page()
    
    # Título Principal
    pdf.set_font("helvetica", style="B", size=16)
    pdf.cell(0, 10, f"Ata de Reunião: {ata.assunto}", new_x="LMARGIN", new_y="NEXT", align="C")
    
    # Data
    pdf.set_font("helvetica", style="I", size=10)
    pdf.cell(0, 10, f"Data da Reunião: {ata.data_criacao.strftime('%d/%m/%Y')}", new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.ln(10) # Quebra de linha maior
    
    # Subtítulo
    pdf.set_font("helvetica", style="B", size=12)
    pdf.cell(0, 10, "Tópicos Discutidos / Acordados:", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
    
    # Processa as quebras de linha e cria a lista numerada
    pdf.set_font("helvetica", size=11)
    topicos_separados = ata.topicos.split('\n')
    
    contador = 1
    for linha in topicos_separados:
        linha_limpa = linha.strip()
        if linha_limpa: # Ignora linhas vazias caso você dê enter duas vezes
            # multi_cell garante que se o texto for grande, ele quebra a linha sozinho no PDF
            pdf.multi_cell(0, 8, f"{contador}. {linha_limpa}", new_x="LMARGIN", new_y="NEXT")
            contador += 1
            
    # Gera o PDF em memória para enviar pelo navegador
    pdf_bytes = pdf.output()
    
    return send_file(
        io.BytesIO(pdf_bytes),
        as_attachment=True,
        download_name=f"Ata_Reuniao_{ata.data_criacao.strftime('%d-%m-%Y')}.pdf",
